...
function currentLocationOptionLabel(username) {
...
function refreshFromSelect() {
    ...
    const employeeSel = document.getElementById("f-employee");
    if (!fromSel)
        return;
    ...
    if (selectedUsername) {
        ...
        if (label)
            currentLocOption = `<option value="${CURRENT_LOCATION_VALUE}">${escapeHtml(label)}</option>`;
    }
...
function renderLocationList() {
    ...
    list.innerHTML = locations
        .slice()
        .sort((a, b) => a.name.localeCompare(b.name))
        .map((l) => `<li><span>${escapeHtml(l.name)}</span><span class="coords">${l.lat.toFixed(4)}, ${l.lng.toFixed(4)}</span></li>`)
        .join("");
}
...
function statusOptionsHtml(current) {
    const statuses = ["assigned", "en_route", "done", "cancelled"];
    return statuses
...
async function loadBoard() {
    ...
    const tasks = await fetchJSON(`/api/tasks?date=${dateInput.value}`);
    if (tasks.length === 0) {
        renderEmptyBoard();
        return;
    }
...
function handleTaskCreated(task) {
    if (task.task_date !== currentBoardDate())
        return;
    const body = document.getElementById("board-body");
...
let employeesState = {};
let pickMode = "from";
function jitterForOfflineEmployee(username) {
...
let APP_EMPLOYEE_ORDER_CACHE = {};
function locationDivIcon() {
    return L.divIcon({
        className: "",
...
function employeeDivIcon(online) {
    return L.divIcon({
        className: "",
...
function renderLocationMarkersOnLiveMap() {
    locations.forEach((loc) => {
        if (locationMarkers[loc.name])
            return;
        const marker = L.marker([loc.lat, loc.lng], { icon: locationDivIcon() }).addTo(liveMap);
        marker.bindPopup(`<p class="popup-title">${escapeHtml(loc.name)}</p><span class="popup-meta">${loc.lat.toFixed(4)}, ${loc.lng.toFixed(4)}</span>`);
        marker.on("click", () => {
            const targetSelect = document.getElementById(pickMode === "from" ? "f-from" : "f-to");
...
function employeePopupHtml(emp) {
...
function upsertEmployeeMarker(emp) {
    ...
    if (employeeMarkers[emp.username]) {
        ...
        marker.on("popupopen", () => {
            const btn = document.querySelector(`.popup-btn[data-assign="${emp.username}"]`);
            btn === null || btn === void 0 ? void 0 : btn.addEventListener("click", () => assignFormTo(emp.username));
        });
        employeeMarkers[emp.username] = marker;
    }
    renderRoster();
    const employeeSel = document.getElementById("f-employee");
    if (employeeSel && employeeSel.value === emp.username)
        refreshFromSelect();
}
function assignFormTo(username) {
    var _a;
    const select = document.getElementById("f-employee");
    select.value = username;
    liveMap.closePopup();
    document.getElementById("f-person").focus();
    (_a = document.getElementById("task-form")) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: "smooth", block: "center" });
}
...
function setupPickModeButtons() {
    document.querySelectorAll(".pick-mode-btn").forEach((btn) => {
...
function setupForm() {
    ...
    swapBtn.addEventListener("click", () => {
        const fromSel = document.getElementById("f-from");
        const toSel = document.getElementById("f-to");
        if (fromSel.value === CURRENT_LOCATION_VALUE)
            return;
        const tmp = fromSel.value;
        fromSel.value = toSel.value;
        toSel.value = tmp;
    });
    document.getElementById("f-employee").addEventListener("change", refreshFromSelect);
    form.addEventListener("submit", async (e) => {
        ...
        const taskDate = document.getElementById("f-date").value;
        const taskTime = document.getElementById("f-time").value;
        ...
        try {
            ...
            document.getElementById("f-employee").selectedIndex = 0;
            refreshFromSelect();
    ...
    document.getElementById("board-body").addEventListener("change", async (e) => {
    ...
    document.getElementById("board-body").addEventListener("click", async (e) => {
        const target = e.target;
...
function resolveFromValue(fromValue, employeeUsername) {
    if (fromValue !== CURRENT_LOCATION_VALUE)
        return fromValue;
...
function setupTabs() {
    ...
    buttons.forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            document.getElementById("tab-assign").style.display = target === "assign" ? "" : "none";
            document.getElementById("tab-locations").style.display = target === "locations" ? "" : "none";
            if (target === "locations")
                initMapPicker();
        });
    });
...
async function initManager() {
    ...
    socket.on("employee_location_updated", (emp) => upsertEmployeeMarker(emp));
