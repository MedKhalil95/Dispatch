// Compiled to static/js/delegation.js
// NOTE: all top-level names here are prefixed deleg* — every .ts file in
// this project compiles together into one shared (non-module) program,
// so names must stay unique across files even though manager.js,
// employee.js, and delegation.js are never loaded on the same page.

let delegEmployeesState: Record<string, EmployeeLocationRecord> = {};
...
function delegJitterForOfflineEmployee(username: string): { lat: number; lng: number } {
...
function delegEscapeHtml(s: string): string {
  const div = document.createElement("div");
  div.textContent = s;
  return div.innerHTML;
}

function delegEmployeePopupHtml(emp: EmployeeLocationRecord): string {
...
function delegBindPopupDirectionsButton(username: string) {
  const btn = document.querySelector(`.directions-btn[data-directions-username="${username}"]`) as HTMLButtonElement | null;
  btn?.addEventListener("click", () => delegOpenDirectionsFor(username));
}

function delegUpsertEmployeeMarker(emp: EmployeeLocationRecord) {
...
function delegOpenDirectionsFor(username: string) {
...
function delegTaskRowHtml(task: TaskRecord): string {
...
const delegRowsById: Record<number, HTMLTableRowElement> = {};

function delegCurrentBoardDate(): string {
...
async function delegLoadBoard() {
  ...
  tasks.forEach((task) => {
    const row = body.querySelector(`tr[data-id="${task.id}"]`) as HTMLTableRowElement | null;
    if (row) delegRowsById[task.id] = row;
  });
}
...
function delegRemoveRow(taskId: number) {
  ...
  const body = document.getElementById("board-body") as HTMLTableSectionElement;
  if (body.children.length === 0) delegRenderEmptyBoard();
}
...
function delegSetupTable() {
  const body = document.getElementById("board-body") as HTMLElement;
  body.addEventListener("click", (e) => {
    const target = e.target as HTMLElement;
    const btn = target.closest(".directions-btn") as HTMLButtonElement | null;
